...
零账号。原理和坑见 `references/setup-form.md`。

**先问一句：这个邮箱之前用 FormSubmit 激活过吗**（比如上一个项目就在收报名）？
激活过就**直接复用那串随机码、跳过本步**——随机码跟**邮箱**绑定，不跟项目绑，
多个项目共用一个端点，靠邮件标题里的产品名分流（`_subject` 就是干这个的）。
一个邮箱只发一次激活信：对已激活邮箱再跑 `--activate` 不会有新激活信，
那次提交会被当成一条普通报名进收件箱。
